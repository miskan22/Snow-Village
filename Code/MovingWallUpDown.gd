extends KinematicBody2D

export var move_speed = 0.5
export var move_distance = -300.0
export var move_direction = Vector2(0, -1) # Up unit vector. Y is down in 2D, so this vector points -Y.

var time_since_init = 0
var origin = Vector2(0, 1) # Down unit vector. Y is down in 2D, so this vector points +Y.

# Called when the node enters the scene tree for the first time.
func _ready():
	time_since_init = 0.0
	origin = position
	pass # Replace with function body.


# Called every frame. 'delta' is the elapsed time since the previous frame.
func _physics_process(delta):
	time_since_init = time_since_init + delta
	var position_on_curve = sin(time_since_init * PI * move_speed)
	var offset = move_distance * position_on_curve * move_direction
	position = origin + offset
	pass
